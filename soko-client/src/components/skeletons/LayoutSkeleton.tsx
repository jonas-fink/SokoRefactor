const SideBarSkeleton = () => (
    <div className="flex flex-col p-8 gap-8 bg-surface min-h-screen animate-pulse">
        <div className="flex items-center gap-2">
            <div className="w-22 h-22 rounded-full bg-surface-2" />
            <div className="h-6 w-20 rounded bg-surface-2" />
        </div>
        <div className="flex flex-col gap-2">
            {Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="h-11 rounded-xl bg-surface-2" />
            ))}
        </div>
    </div>
);
    ...
    return (
        <div className="grid grid-cols-4 min-h-screen antialiased gap-8">
            ...
            <div className="col-span-3 pt-8 animate-pulse">
                <div className="h-8 w-1/3 rounded bg-surface-2 mb-6" />
                <div className="grid grid-cols-3 gap-6">
                    {Array.from({ length: 6 }).map((_, i) => (
                        <div
                            key={i}
                            className="aspect-video rounded-xl bg-surface-2"
                        />
                    ))}
                </div>
            </div>
